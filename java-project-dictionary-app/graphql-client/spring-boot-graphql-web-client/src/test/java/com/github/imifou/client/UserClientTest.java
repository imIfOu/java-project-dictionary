...
public class UserClientTest {
    ...
    void testGetUserById_whenResponseIsErrorWithLocationData() {
        ...
        GraphQLErrorsException exception = assertThrows(GraphQLErrorsException.class, () -> userClient.getUser(USER_ID).block());
        var error = exception.getErrors().get(0);
        assertEquals(error.getExtensions().get("code"), HttpStatus.NOT_FOUND.name());
    ...
    void testCreateUser_whenResponseIsErrorWithLocationData() {
        ...
        var user = buildDefaultUser();
        GraphQLErrorsException exception = assertThrows(GraphQLErrorsException.class, () -> userClient.createUser(user).block());
        var error = exception.getErrors().get(0);
        assertEquals(error.getExtensions().get("code"), HttpStatus.NOT_FOUND.name());
        assertEquals(error.getMessage(), HttpStatus.NOT_FOUND.getReasonPhrase());
    }
    ...
    void testUpdateUser_whenResponseIsErrorWithLocationData() {
        ...
        var user = buildDefaultUser();
        GraphQLErrorsException exception = assertThrows(GraphQLErrorsException.class, () -> userClient.updateUser(USER_ID, user).block());
        var error = exception.getErrors().get(0);
        assertEquals(error.getExtensions().get("code"), HttpStatus.NOT_FOUND.name());
        assertEquals(error.getMessage(), HttpStatus.NOT_FOUND.getReasonPhrase());
    }
